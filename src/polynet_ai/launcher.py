        ...
        "sim-paper": LaunchProfile(
            ...
            fields=[
                ...
                LaunchField(
                    ...
                    max_value=1000000,
                    step=10,
                    example="200 表示用 200 USDC 等名义资金开跑",
                ...
                LaunchField(
                    ...
                    kind="select",
                    default="fixed",
                    description="fixed=每周期固定下注本金并累计盈亏曲线；cumulative=跨周期滚存账户资金。",
                    cli_flag="--capital-reset-mode",
                    options=["fixed", "cumulative"],
                    example="fixed 与批量回放脚本默认一致",
                    detail="与 scripts/run_recorded_live_paper.py 的 --capital-reset-mode 一致。",
                ),
                LaunchField(
                    name="max_cycles",
                    label="测试周期数",
        ...
        "market-paper": LaunchProfile(
            ...
            fields=[
                ...
                LaunchField(
                    ...
                    cli_flag="--capital-reset-mode",
                    options=["fixed", "cumulative"],
                    example="fixed 与实盘验证脚本默认一致",
                    detail="与 scripts/run_polymarket_live_paper.py 的 --capital-reset-mode 一致。",
                ),
                LaunchField(
                    name="record_orderbook_top",
                    label="落盘口买一卖一",
                    kind="checkbox",
                    default=True,
                    description="将 UP/Down 买一卖一价格和挂单量写入原始事件流。",
                    cli_flag="--record-orderbook-top",
                    example="勾选后 ws_trade_events 会带上盘口快照字段",
                    detail="默认开启。需要可用的 CLOB client；若当前环境没有可用凭证，运行时会自动跳过，不影响主流程。",
                ),
                LaunchField(
                    name="orderbook_refresh_seconds",
                    label="盘口快照刷新间隔",
                    kind="number",
                    default=0.5,
                    description="拉取盘口快照的最小刷新间隔（秒）。",
                    cli_flag="--orderbook-refresh-seconds",
                    min_value=0,
                    max_value=10,
                    step=0.1,
                    example="0.5 表示最多每 0.5 秒刷新一次买一卖一",
                    detail="0 表示每条事件都重新拉取盘口，最接近实时但请求压力最高；建议实盘验证从 0.5 或 1 开始。",
                ),
            ],
        ),
    }
